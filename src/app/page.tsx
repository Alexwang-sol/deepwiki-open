...
export default function Home() {
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<'github' | 'gitlab' | 'bitbucket'>('gitlab');
  ...
  // State for welcome section collapse
  const [isWelcomeCollapsed, setIsWelcomeCollapsed] = useState<boolean>(true);
  ...
  const handleGenerateWiki = async () => {
    ...
    params.append('type', (type == 'local' ? type : selectedPlatform) || parsedRepo.type);
  ...
  return (
    <div className="min-h-screen paper-texture flex flex-col">
      <header className="w-full bg-[var(--card-bg)] border-b border-[var(--border-color)] shadow-custom-header">
        <div
          className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4">
      ...
      <main className="flex-1 w-full">
        <div
          className="max-w-6xl mx-auto flex flex-col items-center p-4 md:p-8">

          {/* Welcome and Quick Start Section */}
          <div className="w-full max-w-4xl mb-8 bg-[var(--card-bg)] rounded-lg shadow-custom card-japanese p-8 pt-10">
            {/* Header section */}
            <div className="flex items-center justify-between w-full mb-6">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <h2 className="text-2xl font-bold text-[var(--foreground)] font-serif mb-1">{t('home.welcomeTagline')}</h2>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs text-[var(--muted)] hidden sm:inline">{isWelcomeCollapsed ? t('common.showGuide') : t('common.hideGuide')}</span>
                <button
                  onClick={() => setIsWelcomeCollapsed(!isWelcomeCollapsed)}
                  className="p-2 rounded-full hover:bg-[var(--hover-bg)] transition-colors"
                  aria-label={isWelcomeCollapsed ? "Expand" : "Collapse"}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 text-[var(--muted)] transition-transform duration-300 ${isWelcomeCollapsed ? 'rotate-180' : 'rotate-0'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <form onSubmit={handleFormSubmit} className="flex flex-col gap-3 w-full max-w-3xl my-8">
            ...
            {/* Collapsible content */}
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isWelcomeCollapsed ? 'max-h-0' : 'max-h-[2000px]'}`}>

              {/* Quick Start section */}
              <div className="w-full max-w-2xl mx-auto mb-10 bg-[var(--accent-primary)]/5 border border-[var(--accent-primary)]/20 rounded-lg p-5">
                ...
                <div className="grid grid-cols-1 gap-3 text-xs text-[var(--muted)]">
                  ...
                  <div className="bg-[var(--background)]/70 p-3 rounded border border-[var(--border-color)] font-mono overflow-x-hidden whitespace-nowrap">https://git.huya.com/huya_infra/deepwiki-open/-/tree/master/api</div>
                </div>
              </div>

              {/* Visualization section */}
              <div className="w-full max-w-2xl mx-auto mb-8 bg-[var(--background)]/70 rounded-lg p-6 border border-[var(--border-color)]">
          ...
          {/* Conditionally show processed projects */}
      ...
      <footer className="w-full bg-[var(--card-bg)] border-t border-[var(--border-color)] mt-8">
        <div
